...
//     }
// };
const getExportExpense = async (req, res, next) => {
  try {
    ...
    let baseQuery = `
      SELECT 
        ...
        ve.TotalAmount,
        ve.ExpenseStatusId,
        sts.StatusName AS ExpenseStatus,
        ve.createdAt,
        em.ExpModeName
      ...
      AND (emp.FirstName LIKE :searchKey OR emp.LastName LIKE :searchKey OR emp.EMPCode LIKE :searchKey)
      ${filter_query}
      ORDER BY ve.createdAt DESC
    `;

    const result = await sequelize.query(baseQuery, {
      replacements: { EMPCode, searchKey: `%${searchKey}%`, startDate, endDate },
      type: QueryTypes.SELECT,
    });

    // Add manual mapping (simulate ApprovedBy, ApprovedAmount, RejectReason if available)
    ...
    res.status(200).json({
      Status: true,
      ResponseMessage: "Expense export data fetched successfully",
      DataCount: formattedData.length,
      Data: formattedData,
    });
  } catch (error) {
    console.error("getExportExpense error:", error);
    next(error);
  }
};
