...
const getAllExpenseForHr = async (req: RequestType, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        // ✅ HR Filter Logic - Based on ExpenseStatusChangeByHr
        if (hrFilter === 'pending') {
        ...
        // ✅ ONLY show Manager Approved (ExpenseStatusId = 2)
        let query: string = `
            SELECT 
                ve.*, 
                vs.VisitFrom, 
        ...
        console.log("getAllExpenseForHr - Filter Applied:", hrFilter);
...
const approveDisapproveClaim = async (req: RequestType, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        // Update visitexpense - Manager approval (Status: 2=Approved, 3=Rejected)
        const selectQuery = `
            ...
            SET 
                ...
                ExpenseStatusChangeByHr = NULL  -- ✅ Reset HR flag when manager changes status
        ...
        await sequelize.query(selectQuery, {
            replacements: {
                ...
                ExpenseStatusId: isApprove ? "2" : "3", // 2=Manager Approved, 3=Manager Rejected
                ExpenseReqId: ExpenseReqId,
        ...
        if (!res.headersSent) {
            res.status(200).send({
                error: false,
                data: { ExpenseReqId },
                message: isApprove ? 'Expense approved and sent to HR for review.' : 'Expense rejected.'
            });
        }
    } catch (error: any) {
...
`;
        ...
        if (!expenses.length) {
            res.status(400).json({
                error: true,
                message: "No HR released expenses found"
            });
            return;
        }
        ...
        if (!allImages.length) {
            res.status(400).json({
                error: true,
                message: "No images available for HR released expenses"
            });
            return;
        }
