...
const getDashboardDetail = async (req: RequestType, res: Response): Promise<void> => {
  try {
    ...
    const designationData = req?.payload?.DesigId || [];

    // Check if user is Manager(3) or Executive(4)
    const restrictedRoles = [3, 4];

    const designationData: any[] = JSON.parse(req?.payload?.designationData || "[]");

const restrictedRoles = [3, 4];

const isRestrictedUser = designationData.some(
  (d: any) => restrictedRoles.includes(d.DesigId)
);
    ...
    const query1 = `
      ...
      FROM visitsummary vs
      INNER JOIN visitdetails ve ON vs.VisitId = ve.VisitId
      ${isRestrictedUser ? `WHERE ve.EmpCode = :userId AND YEAR(vs.VisitDate) = YEAR(GETDATE())`
