...
const createExpense = async (req: RequestType, res: Response): Promise<void> => {
    try {
        ...
        if (!VisitSummaryId) {
            return res.status(401).json({ error: true, message: "Visit is not present" });
        }
        ...
        const checkQuery = `
            SELECT * FROM dbo.visitexpense 
            WHERE EmpCode = :EMPCode 
            AND VisitId = :VisitSummaryId 
            AND expensemodeid = :ExpModeId
        `;
        ...
        if (checkResult.length > 0) {
            return res.status(401).json({
                error: true,
                message: "Expense already exists for this mode"
            });
        }

        // ✅ Insert main expense
        const mainInsert = `
            INSERT INTO dbo.visitexpense (
                ExpenseReqId, EmpCode, ConvModeId, amount,
                expensemodeid, VisitId, VisitSummaryId, VisitRemarks,
                Expense_document, Rate, Reason, Distance
            ) VALUES (
                :uuid, :EmpCode, :ConvModeId, :Amount,
                :ExpModeId, :VisitSummaryId, :VisitSummaryId, :Remarks,
                :Doc, :Rate, :Reason, :Distance
            )
        `;

        await sequelize.query(mainInsert, {
            replacements: {
                uuid,
                EmpCode: req?.payload?.appUserId,
                ConvModeId,
            ...
            if (file.file) {
                ...
                const docInsert = `
                    INSERT INTO dbo.expensedocs (
                        ExpenseDocId, ExpenseReqId, Amount,
                        imageName, isVerified, isActive
                    ) VALUES (
                        :uuid1, :uuid, :Amount, :fileName, 'InProgress', 1
                    )
                `;

                await sequelize.query(docInsert, {
                    replacements: {
                        uuid1,
                        uuid,
...
const getAllExpense = async (req: RequestType, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        if (req.query.EMPCode === "all") searchEMPCode = '';
        ...
        const baseQuery = `
            SELECT 
                ve.*, vs.VisitFrom, vs.VisitTo,
                (SELECT CONCAT(e.FirstName,' ',e.LastName) 
                 FROM dbo.employeedetails e 
                 WHERE e.EMPCode = ve.ApprovedById) AS ApprovedByName,
                (SELECT SUM(CAST(ed.Amount AS INT)) 
                 FROM dbo.expensedocs ed 
                 WHERE ed.ExpenseReqId = ve.ExpenseReqId 
                 AND ed.isVerified = :verifyType) AS TotalApproveAmount,
                emp.FirstName, emp.LastName, emp.EMPCode,
                vs.VisitPurpose, em.ExpModeDesc,
                sts.Description AS ExpenseStatus
            ...
            WHERE 
                ...
                AND ve.isActive = 1
                ${filter_query}
                ${DesigId === '4' ? `AND ve.EmpCode = :EMPCode` : `AND (emp.MgrEmployeeID = :EMPCode OR ve.EmpCode = :EMPCode)`}
            ORDER BY ve.createdAt DESC
        `;
        ...
        if (pageIndex == 0) {
            const rowsCount = await sequelize.query(baseQuery, {
                replacements: {
                    searchKey,
                    EMPCode: req?.payload?.appUserId,
                    searchEMPCode,
        ...
        result.rows = await sequelize.query(paginatedQuery, {
            replacements: {
                searchKey,
                EMPCode: req?.payload?.appUserId,
                searchEMPCode,
        ...
        const transformed = result.rows.map((row: any) => ({
            ...row,
            Expense_document: JSON.parse(row.Expense_document || "[]")
        }));
...
const getExportExpense = async (req: RequestType, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        const base = `
            ...
            FORMAT(ve.createdAt AT TIME ZONE 'UTC' AT TIME ZONE 'India Standard Time','dd-MM-yyyy') AS ExpenseDate,
            vs.VisitFrom,
            FORMAT(vs.VisitDate AT TIME ZONE 'UTC' AT TIME_ZONE 'India Standard Time','dd-MM-yyyy') AS VisitDate,
            vs.VisitTo,
            vs.VisitPurpose AS Purpose,
            ve.ApprovedById AS ApprovalID,
            ve.approve_amount AS ApprovedAmount
        ...
        const query = `
            ...
            WHERE 
                (emp.FirstName LIKE :searchKey OR emp.LastName LIKE :searchKey OR emp.EMPCode LIKE :searchKey)
                AND ${DesigId === '4' ? `ve.EmpCode = :EMPCode` : `(emp.MgrEmployeeID = :EMPCode OR ve.EmpCode = :EMPCode)`}
                AND ve.isActive = 1
                ${filter_query}
            ORDER BY ve.createdAt DESC
        `;
...
/* ---------------------------------------------------------
   ✅ GET EXPENSE BY ID
--------------------------------------------------------- */
const getExpenseById = async (req: RequestType, res: Response, next: NextFunction) => {
    try {
        const ExpenseReqId = req.query.ExpenseReqId;

        const query = `
            SELECT 
                vte.*, emp.FirstName, emp.LastName,
                em.ExpModeDesc, 
                vts.VisitDate, vts.VisitFrom, vts.VisitTo, vts.VisitPurpose,
                mst.Description AS expense_status,
                '${req?.payload?.appUserId}' AS loginId
            FROM dbo.visitexpense vte
            INNER JOIN dbo.employeedetails emp ON emp.EMPCode = vte.EmpCode
            INNER JOIN dbo.mststatus mst ON mst.StatusId = vte.ExpenseStatusId
            INNER JOIN dbo.visitsummary vts ON vts.VisitSummaryId = vte.VisitId
            LEFT JOIN dbo.mstexpmode em ON em.ExpModeId = vte.expensemodeid
        ...
        const result = await sequelize.query(query, {
            replacements: { ExpenseReqId },
            type: QueryTypes.SELECT
        });

        const docs = await sequelize.query(
            `SELECT * FROM dbo.expensedocs WHERE ExpenseReqId = :ExpenseReqId`,
            { replacements: { ExpenseReqId }, type: QueryTypes.SELECT }
        );
...
/* ---------------------------------------------------------
   ✅ Mst Expense Mode
--------------------------------------------------------- */
const expMstMode = async (req: RequestType, res: Response) => {
    try {
        ...
        res.status(200).json({
            ...
            DataCount: rows.length,
            Data: {
                MstExpMode: rows.map((r: any) => ({
                    ddlId: r.ExpModeId,
                    ddlDesc: r.ExpModeDesc
                }))
            }
        });
...
/* ---------------------------------------------------------
   ✅ UPDATE CONVERSION MODE RATE
--------------------------------------------------------- */
const updateConvModeRate = async (req: RequestType, res: Response) => {
    try {
        const { ConvModeId, Rate } = req.body;

        const result = await sequelize.query(`
            UPDATE dbo.mstconvmode SET Rate = :Rate WHERE ConvModeId = :ConvModeId
        `, {
            replacements: { ConvModeId, Rate },
            type: QueryTypes.UPDATE
        });
        ...
        if (result[1] === 0) {
            return res.status(404).json({ message: "ConvMode not found" });
        }

        const rows = await sequelize.query(`SELECT * FROM dbo.mstconvmode WHERE IsActive = 1`, {
            type: QueryTypes.SELECT
        });
...
/* ---------------------------------------------------------
   ✅ UPLOAD EXPENSE DOC
--------------------------------------------------------- */
const uploadExpenseDoc = async (req: RequestType, res: Response) => {
    try {
        res.status(200).json({
            message: 'File uploaded successfully'
        });
