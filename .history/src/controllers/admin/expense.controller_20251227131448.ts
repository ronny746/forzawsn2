    ...
    try {
        ...
        for (const expense of expenses) {
            try {
                ...
                // Update documents only if they exist
                if (docs.length > 0) {
                    ...
                    // If no documents but expense exists, still approve/reject it
                    console.log("No documents found, but processing expense:", expense.ExpenseReqId);
                }

                const statusText = isApprove ? 'approved' : 'rejected';
                approvalDetails.push({
                    ExpenseReqId: expense.ExpenseReqId,
                    ExpenseDocId: docs.length > 0 ? docs[0]?.ExpenseDocId : 'N/A',
                    EmployeeName: expense.EmployeeName,
                    EMPCode: expense.EmpCode,
                    Amount: expense.TotalAmount,
                    Status: 'Success',
                    Message: `Expense ${statusText} successfully${docs.length === 0 ? ' (no documents)' : ''}`
                });
