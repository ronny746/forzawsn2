...
const generateDetailedExpenseReport = async (
    ...
    try {
        ...
        let grandTotal = 0;
        reportData.forEach(t => grandTotal += t.totalAmount);
        ...
        doc.text(`Department: ${employee.DesigId || "N/A"}`, 50);
        ...
        doc.font("Helvetica-Bold")
            .text("Expense Type", startX + 5, y + 5)
            .text("Subtype", startX + col1 + 5, y + 5)
            .text("Amount", startX + col1 + col2 + 5, y + 5);
        ...
        reportData.forEach(exp => {
            doc.rect(startX, y, 500, rowHeight).stroke();
            doc.text(exp.type, startX + 5, y + 5);
            y += rowHeight;

            exp.subtypes.forEach(sub => {
                doc.rect(startX, y, 500, rowHeight).stroke();
                doc.text("", startX + 5, y + 5);
                doc.text(sub.name, startX + col1 + 5, y + 5);
                doc.text(sub.amount.toString(), startX + col1 + col2 + 5, y + 5);
                y += rowHeight;
            });
        });
        ...
        doc.rect(startX, y, 500, rowHeight).stroke();
        doc.font("Helvetica-Bold");
        doc.text("Total Amount", startX + col1 + 5, y + 5);
        doc.text(grandTotal.toString(), startX + col1 + col2 + 5, y + 5);

        y += 30;

        doc.fontSize(10)
            .text(`Total Expense for the Month of ${getMonthName(startDate.toString())}: ${grandTotal}`, startX);
