...
const getAllExpense = async (req: RequestType, res: Response): Promise<void> => {
    ...
    `;
        ...
        const queryDocs = `
      SELECT ed.ExpenseReqId, ed.ExpenseDocId, ed.imageName, ed.Amount, ed.isVerified
      FROM dbo.expensedocs ed
      INNER JOIN dbo.visitexpense ve ON ed.ExpenseReqId = ve.ExpenseReqId
      WHERE ve.VisitId = :VisitSummaryId
    `;
        ...
        const expenseDocs: any[] = await sequelize.query(queryDocs, {
            replacements: { VisitSummaryId },
            type: QueryTypes.SELECT,
        });

        // Map docs to respective expenses
        const docsMap: Record<string, any[]> = {};
        expenseDocs.forEach(doc => {
            if (!docsMap[doc.ExpenseReqId]) docsMap[doc.ExpenseReqId] = [];
            docsMap[doc.ExpenseReqId].push(doc);
        });

        // Calculate totals and attach docs
        let totalApproved = 0;
        let totalRejected = 0;

        const processedExpenses = expenses.map(exp => {
            const docs = docsMap[exp.ExpenseReqId] || [];
            console.log(docs, "docs for ", exp.ExpenseReqId);

            // Determine the status from DB
            const status = (exp.ExpenseStatus || "").toLowerCase(); // use actual DB value

            // Calculate approved amount from expenseDocs
            const approvedAmount = docs
                .filter(doc => doc.isVerified?.toLowerCase() === "approved")
                .reduce((sum, doc) => sum + Number(doc.Amount || 0), 0);

            // Update overall totals based on actual expenseDocs approved amounts
            totalApproved += approvedAmount;

            // Rejected amounts could be sum of docs marked rejected or main expense if status rejected
            if (status === "rejected") totalRejected += Number(exp.Amount || 0);

            return {
                ...exp,
                ExpenseStatus: exp.ExpenseStatus, // do not overwrite
        ...
        console.error(error.message, "error in getAllExpense");
