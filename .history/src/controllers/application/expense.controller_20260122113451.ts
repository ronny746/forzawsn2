...
const updateExpense = async (req: RequestType, res: Response): Promise<void> => {
    try {
        const {
            ExpenseReqId,
            ConvModeId,
        ...
        console.log(req.body, "update expense payload data");

        const decoded: any = req?.payload;

        // ✅ Validation: Check if ExpenseReqId is provided
        if (!ExpenseReqId) {
            res.status(422).json({ 
                error: true,
                message: "ExpenseReqId is required" 
            });
            return;
        }
        ...
        const existingExpense: any = await sequelize.query(checkExpenseQuery, {
            replacements: { 
        ...
        if (expenseStatus === 2 || expenseStatus === 3) {
            res.status(400).json({
                error: true,
                message: "Cannot update expense that is already approved or rejected"
            });
            return;
        }
        ...
        if ((!Data || Data.length === 0 || !Data[0].image) && 
            Number(ExpModeId) !== 9 && Number(ExpModeId) !== 7) {
            res.status(422).json({
                error: true,
                message: "Image is required for this expense"
            });
        ...
        if ((Data.length > 0) && (Number(sum) !== Number(Amount))) {
            res.status(422).json({
                error: true,
                message: "Sum of individual amounts must equal total expense amount"
            });
            return;
        }

        // ✅ Update expense in visitexpense table
        const updateExpenseQuery = `
            UPDATE dbo.visitexpense 
            SET 
                ConvModeId = :ConvModeId,
                amount = :Amount,
                expensemodeid = :ExpModeId,
        ...
        await sequelize.query(updateExpenseQuery, {
            replacements: {
                ...
                ExpModeId: ExpModeId ? ExpModeId : 6,
                Remarks: Remarks,
                ExpenseDocument: JSON.stringify(Data),
                Rate: Rate,
                Reason: Reason,
                ExpenseReqId: ExpenseReqId
            },
        ...
        // ✅ Delete existing expense documents
        const deleteDocsQuery = `
            DELETE FROM dbo.expensedocs 
            WHERE ExpenseReqId = :ExpenseReqId
        `;
        ...
        // ✅ Insert updated expense documents
        const promises = Data.map(async (item: any) => {
        ...
        // ✅ Handle special expense modes (DA/TA - 9, 7)
        if (Number(ExpModeId) === 9 || Number(ExpModeId) === 7) {
            const uuid1 = uuidv4();
            const insertQuery1 = `
                INSERT INTO dbo.expensedocs (
        ...
        if (emailGetData[0]?.managerEmail) {
