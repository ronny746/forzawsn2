...
const getAllExpense = async (req: RequestType, res: Response): Promise<void> => {
    try {
        ...
        // Fetch all expenses for the visit
        const query = `
            SELECT 
                ve.ExpenseReqId,
                ve.amount AS Amount,
                ...
                vs.VisitPurpose,
                (SELECT CONCAT(emp.FirstName, ' ', emp.LastName) 
                 FROM dbo.employeedetails emp WHERE emp.EMPCode = ve.ApprovedById) AS ApprovedBy,
                (SELECT CONCAT(emp.FirstName, ' ', emp.LastName) 
                 FROM dbo.employeedetails emp WHERE emp.EMPCode = ve.CheckedById) AS CheckedBy,
                em.ExpModeDesc,
                (SELECT ed.ExpenseDocId, ed.imageName, ed.Amount, ed.isVerified
                 FROM dbo.expensedocs ed WHERE ed.ExpenseReqId = ve.ExpenseReqId
        ...
        const processedExpenses = rows.map(item => {
            const docs = item.expenseDocs ? JSON.parse(item.expenseDocs) : [];

            // Sum approved amounts from expenseDocs
            const approvedAmount = docs
                .filter((d: any) => d.isVerified?.toLowerCase() === "approved")
                .reduce((sum: number, d: any) => sum + Number(d.Amount || 0), 0);

            totalApproved += approvedAmount;
            if ((item.ExpenseStatus || "").toLowerCase() === "rejected") {
                totalRejected += Number(item.Amount || 0);
            }

            return {
                ExpenseReqId: item.ExpenseReqId,
                Amount: item.Amount,
                ExpenseStatus: item.ExpenseStatus,
                VisitFrom: item.VisitFrom,
                VisitTo: item.VisitTo,
                VisitPurpose: item.VisitPurpose,
                ExpModeDesc: item.ExpModeDesc,
                ApprovedBy: item.ApprovedBy,
                CheckedBy: item.CheckedBy,
                expenseDocs: docs,
                ApprovedAmount: approvedAmount,
                canResubmit: (item.ExpenseStatus || "").toLowerCase() === "rejected"
            };
        });
        ...
        console.error(error, "Error in getAllExpense");
